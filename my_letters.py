...
def make_char_ims(font_path, output_height):
    ...
    for c in CHARS:
        ...
        draw.text((0, 0), c, (int(255 * random.random()), int(255 *random.random()), 255), font=font)
...
def make_affine_transform(from_shape, to_shape, 
    ...
    to_size = numpy.array([[to_shape[1], to_shape[0]+30]]).T
...
def generate_bg(num_bg_images):
    ...
    while not found:
        try:
            ...
            #fname = "bg_super/{:08d}.png".format(random.randint(0, num_bg_images - 1))
            fname = "bg_super/bw.jpg"
...
def generate_im(char_ims, num_bg_images):
    ...
    out = cv2.resize(out, (OUTPUT_SHAPE[1]+50, OUTPUT_SHAPE[0]))

    out += numpy.random.normal(scale=0.05, size=out.shape)
    out = numpy.clip(out, 0.0, 1.)
...
def generate_ims():
    ...
    num_bg_images = len(os.listdir("./bg_super"))
    while True:
...
if __name__ == "__main__":
    ...
    for img_idx, (im, c, p) in enumerate(im_gen):
        fname = "super/00000{:0d}_{}_{}.png".format(img_idx, c,
                                               "1" if p else "0")
        print (fname)
